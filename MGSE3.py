...
__usage__ = """
					...
					--ignore <DEACTIVATE BLACKLISTING OF CONTIGS WITH HIGH COVERAGE VALUES> [FALSE]
					...
					WARNING: high coverage contigs are black listed by default. Use --ignore to disable black listing.
...
def get_avg_cov( coverage, genes, output_folder, plotting_status, black_user_specified, black_status, excl_use_spcf_seq, blf):
	...
	if black_status:  # calculation with reference genes when --ignore not in arguments
			...
			for element in excl_use_spcf_seq:
				for k in range(len(genes) - 1, -1, -1):
					if genes[k]['chr'] == element:
						del genes[k]
		else:# removing genes on blacklisted contigs when --ignore and --black not in arguments
...
def main( arguments ):
	...
	if '--black' in arguments:
		...
		if '--ignore' in arguments:	#disable black listing of contigs with high coverage values
			black_status = False
			excl_use_spcf_seq = []
		else:
			black_status = True
	...
	with open( report_file, "w" ) as out: 
		for cov_file in cov_files:
			...
			if not use_ref_genes:# calculation based on all positions
				if black_status:# calculation based on all positions when --ignore is not given
					if '--black' not in arguments:# calculation based on all positions when --black not in arguments
						...
						avg_coverage_median = get_median(values)
						avg_coverage_mean = get_mean(values)
				else:# calculation based on all positions when --ignore is given
					values = list(sorted([x for chromosome in coverage.values() for x in chromosome]))
					avg_coverage_median = get_median(values)
					avg_coverage_mean = get_mean(values)
